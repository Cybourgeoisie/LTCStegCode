...
{
	// Ring Nodes
	memcpy(ring_nodes->nodes, (const unsigned char [NUM_RING_NODES]){27,13,7,14,19,10,18,0,25,19,1,2,22,14,24,2,22,8,26,1,25,0,20,0,11,0,8,4,8,14,8,12,19,25,14,21,20,4,16,27,22,27,25,24,0,4,24,5,1,1,22,4,9,14,7,2,13,6,8,13,16,18,22,14,16,14,20,0,26,19,26,25,22,25,2,1,2,25,22,25,17,3,9,18,2,13,8,8,21,14,24,17,21,0,25,19,16,0,25,22,26,25,1,1,2,0,1,11,13,13,10,13,7,4,11,22,21,14,27,14,16,20,16,2,19,27,24,1,2,3,0,1,19,25,27,5,17,2,7,22,14,5,9,21,12,11,25,21,27,18,15,0,22,1,22,22,0,6,4,26,19,24,4,1,14,1,8,16,2,10,7,18,17,3,22,0,21,11,20,25,17,4,18,26,2,26,11,1,25,25,22,1,9,3,4,19,3,13,14,9,23,14,10,18,22,19,5,27,25,14,21,1,24,7,27,5,1,24,4,27,9,3,4,19,7,14,10,20,6,15,16,14,18,21,24,14,16,21,0,25,20,7,21,25,2,25,0,19,11,2,8,17,10,14,2,4}, NUM_RING_NODES*sizeof(unsigned char));
...
void findPathsToRingNode(int current_ring_node, ring * ring_nodes, char x[3][6])
{
	...
	while (!states_to_evaluate.empty())
	{
		...
		// Walk forward four times, if we hit the necessary node, then add that state
		for (int k = 0; k < NUM_GRAPH_NODES; k++)
		{
			// Aw yis
			int current_graph_node = k;

			// Make a copy of the original state to manipulate
			state * s_temp = new state();
			memcpy(s_temp, &s, sizeof(state));
			...
			// Walk four times
			for (int j = 0; j < 4; j++) {
				// If the third or fourth aren't in the approved list, fuck it
				if (j == 2 && (current_graph_node != x[0][0] &&
					current_graph_node != x[0][1]))
				{
					current_graph_node = -1;
					break;
				} else if (j == 3 && current_graph_node != x[1][0] &&
					current_graph_node != x[1][1])
				{
					current_graph_node = -1;
					break;
				}
				...
				// Rotate the paddle
				for (int i = 0; i < 4; i++) {
					if (Graph[current_graph_node]->edges[i] == s_temp->paddles[current_graph_node]) {
						// Now we know the index -> move clockwise
						if (i == 3 || Graph[current_graph_node]->edges[i+1] == -1) {
							// Find the last non-negative one
							s_temp->paddles[current_graph_node] = Graph[current_graph_node]->edges[0];
						} else {
							s_temp->paddles[current_graph_node] = Graph[current_graph_node]->edges[i+1];
						}
				...
				if (j < 4) {
					current_graph_node = s_temp->paddles[current_graph_node];
				}
			}

			// Now we're on the fifth node
			if (current_graph_node == ring_nodes->nodes[current_ring_node]) {
				// Good! Keep it.
				next_states_to_evaluate.push(*s_temp);

				/*
				cout << "Walked from " << (k+1) << " (" << (int)Graph[k]->value << ") to " << (current_graph_node+1) << endl;
				cout << "Paddles: ";
				for (int i = 0; i < NUM_GRAPH_NODES; i++) {
					cout << (int)(s.paddles[i]+1) << ", ";
				}
				*/
				...
				if (Graph[k]->value == 1) {
					for (auto &bitwise : *(s_temp->v_ints)) {
						bitwise |= ((unsigned long long int)Graph[k]->value << (63-bit_index));
						//cout << "bits are now " << std::bitset<64>(bitwise) << endl;
					}
				}
			} else {
				// Free this memory
				// Just a reminder that only need to delete following new.
				delete s_temp;
			}
		}
...
int main(int argc, char *argv[])
{
	...
	// If we received a save state, then use it
	// Make first state. Add to queue.
	state * s1 = new state({27,{2,25,0,1,3,2,5,4,7,6,9,8,11,10,13,12,19,18,22,20,19,20,18,22,23,24,21,26}, new vector<unsigned long long int>});
	...
	{
		cout << "Ring node #" << current_ring_node << ": " << (int)ring_nodes.nodes[current_ring_node] << " - " << states_to_evaluate.size() << endl;

		findPathsToRingNode(current_ring_node, &ring_nodes, x[current_ring_node]);
		...
		while (!next_states_to_evaluate.empty())
		{
			...
			// Only include the ints from the last set
				...
				paddles_as_index.compare("01250504020205040907090911101314171816182320192221002125") == 0) {
				for (auto &bitwise : *(s.v_ints)) {
		...
		if (current_ring_node == 255) {
			// Check to see if we landed on the original puzzle state - the encrypted state
			it = reduction_map.find("01250504020205040907090911101314171816182320192221002125");
			if (it == reduction_map.end()) {
				cout << "Not here" << endl;
			} else {
		...
		if (current_ring_node == 255)
		{
			...
			outfile.open("bitwise0.bin", ofstream::binary | fstream::trunc);
			...
			outfile.open("bitwise1.bin", ofstream::binary | fstream::trunc);
			...
			outfile.open("bitwise2.bin", ofstream::binary | fstream::trunc);
			...
			outfile.open("bitwise3.bin", ofstream::binary | fstream::trunc);
